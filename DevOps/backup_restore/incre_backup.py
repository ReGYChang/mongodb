...
if(os.path.isdir("{}/oplog/{}_mongodb_oplog".format(backup_path,keep_backup_time))):
    shutil.rmtree("{}/oplog/{}_mongodb_oplog".format(backup_path,keep_backup_time),ignore_errors=True)
    printlog("[Message]: Remove oplog file one day ago.({})".format(keep_backup_time))
else:
